    	...
    	namespace NDR308TS
		{
			...
			RadioHandler::RadioHandler(bool debug) :
				::LibCyberRadio::Driver::RadioHandler(
					                        /* const std::string& name */ "NDR308TS",
											/* int numTuner */ 8,
											...
											/* bool debug */ debug)
			{
					 ...
					 tuner < (_tunerIndexBase + _numTuner); tuner++)
				{
					...
					_tuners[tuner] = new NDR308TS::TunerComponent(
							/* int index */ tuner,
				...
				{
					_wbddcs[wbddc] = new NDR308TS::WbddcComponent(
							/* int index */ wbddc,
							/* RadioHandler* parent */ this,
							/* bool debug */ _debug,
							/* int dataPort */ 1,
				...
				{
					_nbddcs[nbddc] = new NDR308TS::NbddcComponent(
							/* int index */ nbddc,
							/* RadioHandler* parent */ this,
							/* bool debug */ _debug,
							/* int dataPort */ 1,
                ...
                {
                    _wbddcGroups[group] = new NDR308TS::WbddcGroupComponent(
                            /* int index */ group,
                            /* RadioHandler* parent */ this,
                            /* bool debug */ _debug);
                }
                ...
                {
                    _nbddcGroups[group] = new NDR308TS::NbddcGroupComponent(
                            /* int index */ group,
                            /* RadioHandler* parent */ this,
                            /* bool debug */ _debug);
                }
				...
				{
					_dataPorts[dataPort] = new NDR308TS::DataPort(
							/* int index */ dataPort,
							/* RadioHandler* parent */ this,
							/* bool debug */ _debug,
			...
			{
				this->debug("[NDR308TS::RadioHandler::queryConfiguration] Called\n");
				...
				this->debug("[NDR308TS::RadioHandler::queryConfiguration] Returning\n");
			}

			bool RadioHandler::queryVersionInfo()
			{
				this->debug("[NDR308TS::RadioHandler::queryVersionInfo] Called\n");
				...
				_numWbddc = _numTuner;
				this->debug("[NDR308TS::RadioHandler::queryVersionInfo] Number of tuner boards=%d\n", _numTunerBoards);
				this->debug("[NDR308TS::RadioHandler::queryVersionInfo] Number of tuners=%d\n", _numTuner);
				// Deallocate any tuners and DDCs that don't have a tuner board
				this->debug("[NDR308TS::RadioHandler::queryVersionInfo] Deallocating nonexistent tuners/WBDDCs\n");
				TunerComponentDict::iterator it = _tuners.begin();
				for (; it != _tuners.end(); )
				{
					if ( it->first >= (_tunerIndexBase + _numTuner) )
					{
						this->debug("[NDR308TS::RadioHandler::queryVersionInfo] -- Deallocating tuner=%d\n", it->first);
						delete it->second;
						_tuners.erase(it++);
					}
					else
					{
						this->debug("[NDR308TS::RadioHandler::queryVersionInfo] -- OK tuner=%d\n", it->first);
						it++;
					}
				}
				...
				for (; it2 != _wbddcs.end(); )
				{
					...
					{
						this->debug("[NDR308TS::RadioHandler::queryVersionInfo] -- Deallocating WBDDC=%d\n", it2->first);
						delete it2->second;
						_wbddcs.erase(it2++);
					}
					else
					{
						this->debug("[NDR308TS::RadioHandler::queryVersionInfo] -- OK WBDDC=%d\n", it2->first);
						it2++;
					}
				}
				// Determine frequency range from max bandwidth
				this->debug("[NDR308TS::RadioHandler::queryVersionInfo] Max tuner bandwidth=%d MHz\n", _maxTunerBw);
			...
			// NOTE: The default implementation is the NDR308TS implementation,
