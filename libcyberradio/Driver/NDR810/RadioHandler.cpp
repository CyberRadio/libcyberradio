 ...
 * \brief Defines the radio handler interface for the NDR810.
 * \author DA
 * \author NH
 * \author MN
...
#include "LibCyberRadio/Driver/NDR810/NbddcComponent.h"
    	...
    	namespace NDR810
		{
			...
			RadioHandler::RadioHandler(bool debug) :
				::LibCyberRadio::Driver::RadioHandler(
											...
											/* VitaIfSpec ifSpec */ NDR810::VitaIfSpec(),
											/* bool debug */ debug)
			{
					 ...
					 tuner < (_tunerIndexBase + _numTuner); tuner++)
				{
					...
					_tuners[tuner] = new NDR810::TunerComponent(
							/* int index */ tuner,
				...
				{
					_wbddcs[wbddc] = new NDR810::WbddcComponent(
							/* int index */ wbddc,
							/* RadioHandler* parent */ this,
							/* bool debug */ _debug,
							/* int dataPort */ 1,
				...
				{
					_nbddcs[nbddc] = new NDR810::NbddcComponent(
							/* int index */ nbddc,
							/* RadioHandler* parent */ this,
							/* bool debug */ _debug,
							/* int dataPort */ 1,
                ...
                {
                    _wbddcGroups[group] = new NDR810::WbddcGroupComponent(
                            /* int index */ group,
                            /* RadioHandler* parent */ this,
                            /* bool debug */ _debug);
                }
                ...
                {
                    _nbddcGroups[group] = new NDR810::NbddcGroupComponent(
                            /* int index */ group,
                            /* RadioHandler* parent */ this,
                            /* bool debug */ _debug);
                }
				...
				{
					_dataPorts[dataPort] = new NDR810::DataPort(
							/* int index */ dataPort,
							/* RadioHandler* parent */ this,
							/* bool debug */ _debug,
			...
			{
				this->debug("[NDR810::RadioHandler::queryConfiguration] Called\n");
				...
				this->debug("[NDR810::RadioHandler::queryConfiguration] Returning\n");
			}

			bool RadioHandler::queryVersionInfo()
			{
				this->debug("[NDR810::RadioHandler::queryVersionInfo] Called\n");
				...
				_numWbddc = _numTuner;
				this->debug("[NDR810::RadioHandler::queryVersionInfo] Number of tuner boards=%d\n", _numTunerBoards);
				this->debug("[NDR810::RadioHandler::queryVersionInfo] Number of tuners=%d\n", _numTuner);
				// Deallocate any tuners and DDCs that don't have a tuner board
				this->debug("[NDR810::RadioHandler::queryVersionInfo] Deallocating nonexistent tuners/WBDDCs\n");
				TunerComponentDict::iterator it = _tuners.begin();
				for (; it != _tuners.end(); )
				{
					if ( it->first >= (_tunerIndexBase + _numTuner) )
					{
						this->debug("[NDR810::RadioHandler::queryVersionInfo] -- Deallocating tuner=%d\n", it->first);
						delete it->second;
						_tuners.erase(it++);
					}
					else
					{
						this->debug("[NDR810::RadioHandler::queryVersionInfo] -- OK tuner=%d\n", it->first);
						it++;
					}
				}
				...
				for (; it2 != _wbddcs.end(); )
				{
					...
					{
						this->debug("[NDR810::RadioHandler::queryVersionInfo] -- Deallocating WBDDC=%d\n", it2->first);
						delete it2->second;
						_wbddcs.erase(it2++);
					}
					else
					{
						this->debug("[NDR810::RadioHandler::queryVersionInfo] -- OK WBDDC=%d\n", it2->first);
						it2++;
					}
				}
				// Determine frequency range from max bandwidth
				this->debug("[NDR810::RadioHandler::queryVersionInfo] Max tuner bandwidth=%d MHz\n", _maxTunerBw);
				for (it = _tuners.begin(); it != _tuners.end(); it++)
				{
					it->second->setFrequencyRangeMax(_maxTunerBw * 1e6);
				}
				this->debug("[NDR810::RadioHandler::queryVersionInfo] Returning %s\n", debugBool(ret));
                // Debug dump
                if (ret)
                {
                    for (BasicStringStringDict::iterator it = _versionInfo.begin(); it != _versionInfo.end(); it++)
                    {
                        this->debug("[NDR810::RadioHandler::queryVersionInfo] %s = \"%s\"\n", it->first.c_str(), it->second.c_str());
                    }
                }
				return ret;
			}

			// NOTE: The default implementation is the NDR810 implementation,
